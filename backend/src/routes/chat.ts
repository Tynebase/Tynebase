    ...
    async (request, reply) => {
      try {
        ...
        const readReceiptsMap = new Map(
          (readReceipts || []).map(r => [r.channel_id, r.last_read_at])
        );
  ...
  fastify.post(
    ...
    async (request, reply) => {
      try {
        ...
        const { error } = await supabaseAdmin.rpc('create_default_chat_channels', {
  ...
  fastify.get(
    ...
    async (request, reply) => {
      try {
        ...
        const query = request.query as { limit?: string; before?: string; parent_id?: string };
