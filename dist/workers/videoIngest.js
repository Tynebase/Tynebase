...
exports.processVideoIngestJob = processVideoIngestJob;
...
const VideoIngestPayloadSchema = zod_1.z.object({
    storage_path: zod_1.z.string().min(1).optional(),
    original_filename: zod_1.z.string().min(1).optional(),
...
}).refine((data) => data.storage_path || data.youtube_url || data.url, { message: 'Either storage_path, youtube_url, or url must be provided' });
/**
 ...
 */
async function processVideoIngestJob(job) {
    ...
    try {
        ...
        if (isYouTubeVideo) {
            ...
            try {
                ...
                const gcsUri = await (0, gcs_1.uploadToGCS)(localVideoPath, gcsFileName, 'audio/mpeg');
                ...
                const geminiAudioResult = await (0, vertex_1.transcribeAudio)(gcsUri, 'Transcribe this audio content with timestamps. Include all spoken words and important context.');
            ...
            try {
                transcriptionResult = await (0, vertex_1.transcribeVideo)(videoUrl, 'Transcribe this video content with timestamps. Include all spoken words and important visual context. Format the output as a readable transcript with clear sections.');
 ...
 */
async function getSignedVideoUrl(storagePath, workerId) {
    try {
 ...
 * @param workerId - Worker ID for logging
 */
async function deleteVideoFromStorage(storagePath, workerId) {
    try {
 ...
 */
function estimateVideoDuration(transcript, fileSize) {
 ...
 * @param workerId - Worker ID for logging
 */
async function downloadFromSidecar(videoUrl, outputTemplate, workerId) {
 ...
 * @param workerId - Worker ID for logging
 */
async function downloadVideoFromURL(videoUrl, outputTemplate, workerId) {
    const axios = require('axios');
    const basePath = outputTemplate.replace('.%(ext)s', '');
    const outputPath = `${basePath}.mp3`;
    ...
    throw new Error(`Unable to download video from URL: ${videoUrl}. ` +
...
//# sourceMappingURL=videoIngest.js.map