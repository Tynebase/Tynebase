...
exports.default = ragRoutes;
const zod_1 = require("zod");
const tenantContext_1 = require("../middleware/tenantContext");
const auth_1 = require("../middleware/auth");
const membershipGuard_1 = require("../middleware/membershipGuard");
const rateLimit_1 = require("../middleware/rateLimit");
const ingestion_1 = require("../services/rag/ingestion");
const search_1 = require("../services/rag/search");
const chat_1 = require("../services/rag/chat");
const supabase_1 = require("../lib/supabase");
const creditCalculator_1 = require("../utils/creditCalculator");
/**
 * Zod schema for POST /api/rag/ingest request body
 */
const ingestDocumentBodySchema = zod_1.z.object({
 ...
 */
const searchDocumentsBodySchema = zod_1.z.object({
    query: zod_1.z.string().min(1).max(1000),
    limit: zod_1.z.number().int().min(1).max(100).default(10),
    use_reranking: zod_1.z.boolean().default(true),
    rerank_top_n: zod_1.z.number().int().min(1).max(50).default(10),
});
/**
 * Zod schema for GET /api/rag/similar/:chunkId path parameters
 */
const similarChunksParamsSchema = zod_1.z.object({
    chunkId: zod_1.z.string().uuid(),
});
/**
 * Zod schema for GET /api/rag/similar/:chunkId query parameters
 */
const similarChunksQuerySchema = zod_1.z.object({
...
const chatBodySchema = zod_1.z.object({
    ...
    max_context_chunks: zod_1.z.number().int().min(1).max(20).default(10),
...
async function ragRoutes(fastify) {
    ...
    fastify.post('/api/rag/ingest', {
        ...
        try {
            ...
            const result = await (0, ingestion_1.ingestDocument)(document_id, tenant.id, content, metadata);
            if (!result.success) {
                fastify.log.error({ error: result.error, documentId: document_id, tenantId: tenant.id }, 'Document ingestion failed');
    ...
    fastify.post('/api/rag/search', {
        ...
        try {
            ...
            const results = await (0, search_1.searchDocuments)({
                tenantId: tenant.id,
                query,
                limit,
     ...
     * - User must be member of tenant
     */
    fastify.get('/api/rag/similar/:chunkId', {
        preHandler: [rateLimit_1.rateLimitMiddleware, tenantContext_1.tenantContextMiddleware, auth_1.authMiddleware, membershipGuard_1.membershipGuard],
    }, async (request, reply) => {
        try {
            const tenant = request.tenant;
            ...
            const { limit } = query;
            const results = await (0, search_1.findSimilarChunks)(chunkId, tenant.id, limit);
            fastify.log.info({
                tenantId: tenant.id,
                userId: user.id,
                chunkId,
    ...
    fastify.get('/api/rag/stats', {
        ...
        try {
            const tenant = request.tenant;
            const user = request.user;
            const stats = await (0, search_1.getEmbeddingStats)(tenant.id);
            fastify.log.info({
                tenantId: tenant.id,
                userId: user.id,
                stats,
    ...
    fastify.post('/api/rag/reingest', {
        ...
        try {
            ...
            const results = await (0, ingestion_1.reIngestTenantDocuments)(tenant.id);
     ...
     * - User must be member of tenant
     */
    fastify.get('/api/sources/health', {
        preHandler: [rateLimit_1.rateLimitMiddleware, tenantContext_1.tenantContextMiddleware, auth_1.authMiddleware, membershipGuard_1.membershipGuard],
    }, async (request, reply) => {
        try {
            const tenant = request.tenant;
            ...
            const { count: totalDocuments, error: totalError } = await supabase_1.supabaseAdmin
                .from('documents')
                .select('*', { count: 'exact', head: true })
                .eq('tenant_id', tenant.id);
            if (totalError) {
                fastify.log.error({ error: totalError, tenantId: tenant.id }, 'Failed to count total documents');
                return reply.code(500).send({
                    error: {
                        code: 'QUERY_FAILED',
            ...
            const { count: indexedDocuments, error: indexedError } = await supabase_1.supabaseAdmin
                .from('documents')
                .select('*', { count: 'exact', head: true })
                .eq('tenant_id', tenant.id)
                .not('last_indexed_at', 'is', null);
            if (indexedError) {
                fastify.log.error({ error: indexedError, tenantId: tenant.id }, 'Failed to count indexed documents');
                return reply.code(500).send({
                    error: {
                        code: 'QUERY_FAILED',
            ...
            const { data: allIndexedDocs, error: allIndexedError } = await supabase_1.supabaseAdmin
                .from('documents')
                .select('id, title, updated_at, last_indexed_at')
                .eq('tenant_id', tenant.id)
                .not('last_indexed_at', 'is', null);
            if (allIndexedError) {
                fastify.log.error({ error: allIndexedError, tenantId: tenant.id }, 'Failed to query indexed documents for outdated check');
                return reply.code(500).send({
                    error: {
                        code: 'QUERY_FAILED',
            ...
            const { data: failedJobsData, count: failedJobs, error: failedJobsError } = await supabase_1.supabaseAdmin
                .from('job_queue')
                ...
                .eq('status', 'failed');
            if (failedJobsError) {
                fastify.log.error({ error: failedJobsError, tenantId: tenant.id }, 'Failed to count failed jobs');
                return reply.code(500).send({
                    error: {
                        code: 'QUERY_FAILED',
            ...
            const { count: pendingJobs } = await supabase_1.supabaseAdmin
                .from('job_queue')
                .select('*', { count: 'exact', head: true })
                .eq('tenant_id', tenant.id)
                .eq('type', 'rag_index')
            ...
            const { count: processingJobs } = await supabase_1.supabaseAdmin
                .from('job_queue')
                .select('*', { count: 'exact', head: true })
                .eq('tenant_id', tenant.id)
                .eq('type', 'rag_index')
            ...
            const { count: totalChunks } = await supabase_1.supabaseAdmin
            ...
            if (neverIndexedError) {
                fastify.log.error({ error: neverIndexedError, tenantId: tenant.id }, 'Failed to query never-indexed documents');
                return reply.code(500).send({
                    error: {
                        code: 'QUERY_FAILED',
     ...
     */
    fastify.post('/api/sources/:id/reindex', {
        preHandler: [rateLimit_1.rateLimitMiddleware, tenantContext_1.tenantContextMiddleware, auth_1.authMiddleware, membershipGuard_1.membershipGuard],
    }, async (request, reply) => {
        try {
            const tenant = request.tenant;
            ...
            if (user.role !== 'admin') {
                fastify.log.warn({ documentId, userId: user.id, userRole: user.role, tenantId: tenant.id }, 'User attempted to trigger re-index without admin permission');
                return reply.code(403).send({
                    error: {
                        code: 'FORBIDDEN',
            ...
            if (docError) {
                if (docError.code === 'PGRST116') {
                    fastify.log.warn({ documentId, tenantId: tenant.id, userId: user.id }, 'Document not found or access denied for re-index');
                ...
                fastify.log.error({ error: docError, documentId, tenantId: tenant.id, userId: user.id }, 'Failed to fetch document for re-index');
                return reply.code(500).send({
                    error: {
                        code: 'FETCH_FAILED',
            ...
            const { data: existingJobs, error: jobCheckError } = await supabase_1.supabaseAdmin
                ...
                .limit(1);
            if (jobCheckError) {
                fastify.log.error({ error: jobCheckError, documentId, tenantId: tenant.id }, 'Failed to check for existing rag_index jobs');
                return reply.code(500).send({
                    error: {
            ...
            if (existingJobs && existingJobs.length > 0) {
                ...
                fastify.log.info({ documentId, existingJobId: existingJob.id, status: existingJob.status, tenantId: tenant.id }, 'Re-index skipped - job already pending/processing');
                return reply.code(200).send({
                    success: true,
                    data: {
            ...
            const { dispatchJob } = await Promise.resolve().then(() => __importStar(require('../utils/dispatchJob')));
    ...
    fastify.post('/api/ai/chat', {
        ...
        try {
            ...
            if (!hasConsent) {
                fastify.log.warn({ userId: user.id, tenantId: tenant.id }, 'RAG chat blocked: knowledge_indexing consent not granted');
                return reply.code(403).send({
                    error: {
            ...
            const creditsToDeduct = (0, creditCalculator_1.getModelCreditCost)(model || 'deepseek');
            ...
            if (deductError) {
                fastify.log.error({ error: deductError, tenantId: tenant.id }, 'Credit deduction failed for RAG chat');
                return reply.code(500).send({
                    error: {
            ...
            if (!deductResult || deductResult.success === false) {
                ...
                fastify.log.warn({ tenantId: tenant.id, userId: user.id, errorMessage }, 'Credit deduction rejected for RAG chat');
                return reply.code(403).send({
                    error: {
            ...
            if (stream) {
                ...
                try {
                    const chatStream = (0, chat_1.chatWithRAGStream)({
                        tenantId: tenant.id,
                        userId: user.id,
                        query,
                        history,
                    ...
                    if (finalResponse) {
                        ...
                        reply.raw.write(`data: ${JSON.stringify({
                            type: 'citations',
                            citations: finalResponse.citations.map((c) => ({
                        ...
                        await supabase_1.supabaseAdmin.from('query_usage').insert({
                            tenant_id: tenant.id,
                            user_id: user.id,
                            query_type: 'rag_chat',
                            query_text: query,
                ...
                const { chatWithRAG } = await Promise.resolve().then(() => __importStar(require('../services/rag/chat')));
    ...
    fastify.get('/api/sources', {
        ...
        try {
            ...
            const querySchema = zod_1.z.object({
                ...
                status: zod_1.z.enum(['indexed', 'pending', 'outdated', 'failed']).optional(),
            ...
            let docsQuery = supabase_1.supabaseAdmin
                .from('documents')
                .select(`
            id,
            title,
          ...
          `, { count: 'exact' })
            ...
            if (docsError) {
                fastify.log.error({ error: docsError, tenantId: tenant.id }, 'Failed to fetch documents for sources list');
                return reply.code(500).send({
                    error: {
                        code: 'QUERY_FAILED',
            ...
            const { data: failedJobs, error: failedJobsError } = await supabase_1.supabaseAdmin
                .from('job_queue')
                .select('payload')
                .eq('tenant_id', tenant.id)
                .eq('type', 'rag_index')
            ...
            const filteredSources = status
                ? sources.filter((s) => s.indexing_status === status)
                : sources;
     ...
     * - User must be member of tenant
     */
    fastify.get('/api/sources/normalized', {
        preHandler: [rateLimit_1.rateLimitMiddleware, tenantContext_1.tenantContextMiddleware, auth_1.authMiddleware, membershipGuard_1.membershipGuard],
    }, async (request, reply) => {
        try {
            const tenant = request.tenant;
            ...
            const { data: documents, error } = await supabase_1.supabaseAdmin
                .from('documents')
                .select(`
            id,
            title,
          ...
          `)
                ...
                .limit(limit);
            if (error) {
                fastify.log.error({ error, tenantId: tenant.id, userId: user.id }, 'Failed to fetch normalized documents');
                return reply.code(500).send({
                    error: {
                        code: 'FETCH_FAILED',
     ...
     * - User must have 'admin' role
     */
    fastify.post('/api/sources/repair/stuck-jobs', {
        preHandler: [rateLimit_1.rateLimitMiddleware, tenantContext_1.tenantContextMiddleware, auth_1.authMiddleware, membershipGuard_1.membershipGuard],
    }, async (request, reply) => {
        try {
            const tenant = request.tenant;
            ...
            const bodySchema = zod_1.z.object({
                max_age_minutes: zod_1.z.number().int().min(1).max(1440).default(30),
            });
            ...
            const { data: stuckJobs, error: fetchError } = await supabase_1.supabaseAdmin
                .from('job_queue')
            ...
            if (fetchError) {
                fastify.log.error({ error: fetchError, tenantId: tenant.id }, 'Failed to query stuck jobs');
                return reply.code(500).send({
                    error: {
                        code: 'QUERY_FAILED',
            ...
            for (const job of stuckRagIndexJobs) {
                ...
                if (job.attempts >= 3) {
                    const { error: failError } = await supabase_1.supabaseAdmin
                        .from('job_queue')
                        .update({
                        status: 'failed',
     ...
     * - User must have 'admin' role
     */
    fastify.post('/api/sources/repair/orphaned', {
        preHandler: [rateLimit_1.rateLimitMiddleware, tenantContext_1.tenantContextMiddleware, auth_1.authMiddleware, membershipGuard_1.membershipGuard],
    }, async (request, reply) => {
        try {
            const tenant = request.tenant;
            ...
            const { data: indexedDocs, error: indexedError } = await supabase_1.supabaseAdmin
                .from('documents')
                .select('id, title, last_indexed_at')
                .eq('tenant_id', tenant.id)
                .not('last_indexed_at', 'is', null);
            if (indexedError) {
                fastify.log.error({ error: indexedError, tenantId: tenant.id }, 'Failed to query indexed documents');
                return reply.code(500).send({
                    error: {
                        code: 'QUERY_FAILED',
            ...
            let docIdsWithEmbeddings = new Set();
            if (docIds.length > 0) {
                const { data: embeddings, error: embedError } = await supabase_1.supabaseAdmin
                    .from('document_embeddings')
                    .select('document_id')
                    .eq('tenant_id', tenant.id)
            ...
            const orphanedDocs = (indexedDocs || []).filter(doc => !docIdsWithEmbeddings.has(doc.id));
            // Find documents with embeddings but no last_indexed_at (inconsistent state)
            const { data: allDocsNoTimestamp, error: fallbackError } = await supabase_1.supabaseAdmin
                .from('documents')
                .select('id, title')
                .eq('tenant_id', tenant.id)
                .is('last_indexed_at', null);
            if (!fallbackError && allDocsNoTimestamp && allDocsNoTimestamp.length > 0) {
                const { data: embeddingsForUntracked, error: embedCheckError } = await supabase_1.supabaseAdmin
                    .from('document_embeddings')
                    .select('document_id')
                    .eq('tenant_id', tenant.id)
            ...
            for (const doc of orphanedDocs) {
                ...
                const { error: clearError } = await supabase_1.supabaseAdmin
                    .from('documents')
