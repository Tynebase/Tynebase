 ...
 */
const listCollectionsQuerySchema = zod_1.z.object({
    visibility: zod_1.z.enum(['public', 'team', 'private']).optional(),
 ...
 */
const createCollectionBodySchema = zod_1.z.object({
    name: zod_1.z.string().min(1).max(255),
    description: zod_1.z.string().max(2000).optional(),
    cover_image_url: zod_1.z.string().url().optional(),
    color: zod_1.z.string().regex(/^#[0-9A-Fa-f]{6}$/).optional(),
    visibility: zod_1.z.enum(['public', 'team', 'private']).default('private'),
});
/**
 * Zod schema for PATCH /api/collections/:id request body
 */
const updateCollectionBodySchema = zod_1.z.object({
    name: zod_1.z.string().min(1).max(255).optional(),
    description: zod_1.z.string().max(2000).optional(),
    cover_image_url: zod_1.z.string().url().nullable().optional(),
    color: zod_1.z.string().regex(/^#[0-9A-Fa-f]{6}$/).optional(),
    visibility: zod_1.z.enum(['public', 'team', 'private']).optional(),
 ...
 */
const addDocumentsBodySchema = zod_1.z.object({
    document_ids: zod_1.z.array(zod_1.z.string().uuid()).min(1).max(50),
});
/**
 * Zod schema for removing a document from a collection
 */
const removeDocumentParamsSchema = zod_1.z.object({
...
async function collectionRoutes(fastify) {
        ...
        try {
          ...
          `, { count: 'exact' })
            ...
            if (visibility) {
                ...
                const { data: memberCollections } = await supabase_1.supabaseAdmin
            ...
            if (collectionIds.length > 0) {
                const { data: counts } = await supabase_1.supabaseAdmin
     ...
     */
    fastify.get('/api/collections/:id', {
        preHandler: [rateLimit_1.rateLimitMiddleware, tenantContext_1.tenantContextMiddleware, auth_1.authMiddleware, membershipGuard_1.membershipGuard],
    }, async (request, reply) => {
        try {
            const tenant = request.tenant;
            ...
            // Fetch collection
            const { data: collection, error } = await supabase_1.supabaseAdmin
                .from('collections')
                .select(`
            id,
            name,
          ...
          `)
            ...
            const hasAccess = collection.visibility !== 'private' ||
    ...
    fastify.post('/api/collections', {
        ...
        try {
            ...
            const { data: collection, error: createError } = await supabase_1.supabaseAdmin
                .from('collections')
                .insert({
                tenant_id: tenant.id,
                author_id: user.id,
     ...
     * Update a collection
     */
    fastify.patch('/api/collections/:id', {
        preHandler: [rateLimit_1.rateLimitMiddleware, tenantContext_1.tenantContextMiddleware, auth_1.authMiddleware, membershipGuard_1.membershipGuard],
    }, async (request, reply) => {
        try {
            const tenant = request.tenant;
     ...
     * Delete a collection
     */
    fastify.delete('/api/collections/:id', {
        preHandler: [rateLimit_1.rateLimitMiddleware, tenantContext_1.tenantContextMiddleware, auth_1.authMiddleware, membershipGuard_1.membershipGuard],
    }, async (request, reply) => {
        try {
            const tenant = request.tenant;
     ...
     */
    fastify.post('/api/collections/:id/documents', {
        preHandler: [rateLimit_1.rateLimitMiddleware, tenantContext_1.tenantContextMiddleware, auth_1.authMiddleware, membershipGuard_1.membershipGuard],
    }, async (request, reply) => {
        try {
            const tenant = request.tenant;
            ...
            // Verify ownership
            const { data: collection, error: fetchError } = await supabase_1.supabaseAdmin
                .from('collections')
                .select('id, author_id')
                .eq('id', id)
                .eq('tenant_id', tenant.id)
            ...
            const { data: documents } = await supabase_1.supabaseAdmin
                .from('documents')
                .select('id')
                .eq('tenant_id', tenant.id)
     ...
     */
    fastify.delete('/api/collections/:id/documents/:documentId', {
        preHandler: [rateLimit_1.rateLimitMiddleware, tenantContext_1.tenantContextMiddleware, auth_1.authMiddleware, membershipGuard_1.membershipGuard],
    }, async (request, reply) => {
        try {
            const tenant = request.tenant;
     ...
     */
    fastify.get('/api/collections/:id/members', {
        preHandler: [rateLimit_1.rateLimitMiddleware, tenantContext_1.tenantContextMiddleware, auth_1.authMiddleware, membershipGuard_1.membershipGuard],
    }, async (request, reply) => {
        try {
            const tenant = request.tenant;
            ...
            const { data: collection, error: collectionError } = await supabase_1.supabaseAdmin
                .from('collections')
     ...
     */
    fastify.post('/api/collections/:id/members', {
        preHandler: [rateLimit_1.rateLimitMiddleware, tenantContext_1.tenantContextMiddleware, auth_1.authMiddleware, membershipGuard_1.membershipGuard],
    }, async (request, reply) => {
        try {
            const tenant = request.tenant;
            ...
            // Add member
            const { data: member, error: insertError } = await supabase_1.supabaseAdmin
                .from('collection_members')
                .insert({
                collection_id: id,
     ...
     */
    fastify.delete('/api/collections/:id/members/:memberId', {
        preHandler: [rateLimit_1.rateLimitMiddleware, tenantContext_1.tenantContextMiddleware, auth_1.authMiddleware, membershipGuard_1.membershipGuard],
    }, async (request, reply) => {
        try {
            const tenant = request.tenant;
            ...
            if (!zod_1.z.string().uuid().safeParse(id).success || !zod_1.z.string().uuid().safeParse(memberId).success) {
                return reply.code(400).send({
 ...
 */
async function checkCollectionMembership(collectionId, userId) {
