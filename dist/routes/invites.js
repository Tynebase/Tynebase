...
async function findAuthUserByIdOrEmail(userId, email) {
    const normalizedEmail = email.trim().toLowerCase();
    let page = 1;
    const perPage = 200;
    try {
        while (true) {
            ...
            const users = data?.users || [];
            const matchingUser = users.find((authUser) => (userId && authUser.id === userId) || authUser.email?.toLowerCase() === normalizedEmail);
            if (matchingUser) {
                return matchingUser;
            }
            if (users.length < perPage) {
 ...
 */
const inviteRoleSchema = zod_1.z.enum(roles_1.WORKSPACE_ROLE_INPUTS).default('viewer').transform((role) => (0, roles_1.normalizeWorkspaceRole)(role));
const legacyInviteRoleSchema = zod_1.z.enum(roles_1.WORKSPACE_ROLE_INPUTS).transform((role) => (0, roles_1.normalizeWorkspaceRole)(role));
/**
 ...
 */
const acceptInviteSchema = zod_1.z.union([
    zod_1.z.object({
        invite_id: zod_1.z.string().uuid(),
        full_name: zod_1.z.string().trim().max(100, 'Name must be 100 characters or less').optional(),
 ...
 */
async function invitesRoutes(fastify) {
    /**
     * POST /api/invites
    ...
    fastify.post('/api/invites', {
        ...
        try {
            ...
            if (!(0, roles_1.canManageWorkspace)(user.role, user.is_super_admin)) {
            ...
            if (existingAuthUser && !existingAuthUser.email_confirmed_at) {
                const { error: deleteAuthUserError } = await supabase_1.supabaseAdmin.auth.admin.deleteUser(existingAuthUser.id);
                if (deleteAuthUserError) {
                    fastify.log.warn({ error: deleteAuthUserError, email, authUserId: existingAuthUser.id }, 'Failed to delete stale unconfirmed auth user before sending invite');
                }
                else {
                    existingAuthUser = null;
                }
            }
            const { data: existingPendingInvite } = await supabase_1.supabaseAdmin
                .from('workspace_invites')
                .select('id')
                .eq('tenant_id', tenant.id)
                .eq('email', email)
            ...
            const { data: inviteRecord, error: inviteRecordError } = await supabase_1.supabaseAdmin
                .from('workspace_invites')
                .insert({
                tenant_id: tenant.id,
                email,
            ...
            if (existingAuthUser?.id) {
                const { data: existingDbUser } = await supabase_1.supabaseAdmin
                    .from('users')
                    .select('id')
                    .eq('email', email)
                    .maybeSingle();
                if (existingDbUser) {
                    (0, notifications_1.notifyInvitation)({
    ...
    fastify.post('/api/invites/accept', {
        ...
        try {
            ...
            if ('invite_id' in body) {
                const { data: inviteRecord, error: inviteError } = await supabase_1.supabaseAdmin
                    .from('workspace_invites')
                ...
                if (password) {
                    const { error: passwordError } = await supabase_1.supabaseAdmin.auth.admin.updateUserById(authUser.id, {
                        password,
                    });
                    if (passwordError) {
                ...
                await supabase_1.supabaseAdmin.auth.admin.updateUserById(authUser.id, {
                    user_metadata: {
                ...
                await supabase_1.supabaseAdmin
                    ...
                    .eq('id', inviteRecord.id);
                fastify.log.info({ userId: authUser.id, tenantId: inviteRecord.tenant_id, role: inviteRecord.role, inviteId: inviteRecord.id }, 'User accepted invite and joined tenant');
                (0, auditLog_1.writeAuditLog)({
            ...
            const metadataRole = (0, roles_1.normalizeWorkspaceRole)(authUser.user_metadata?.role);
            ...
            if (password) {
                const { error: passwordError } = await supabase_1.supabaseAdmin.auth.admin.updateUserById(user_id, {
                    password,
                });
                if (passwordError) {
            ...
            fastify.log.info({ userId: user_id, tenantId: tenant_id, role }, 'User accepted legacy invite and joined tenant');
            (0, auditLog_1.writeAuditLog)({
                tenantId: tenant_id,
                actorId: user_id,
                action: 'user.invite_accepted',
    ...
    fastify.get('/api/invites/pending', {
        preHandler: [rateLimit_1.rateLimitMiddleware, tenantContext_1.tenantContextMiddleware, auth_1.authMiddleware, membershipGuard_1.membershipGuard],
    }, async (request, reply) => {
        try {
            const tenant = request.tenant;
            ...
            const { data: invites, error } = await supabase_1.supabaseAdmin
                .from('workspace_invites')
            ...
            const pendingInvites = (invites || []).map((invite) => ({
                id: invite.id,
                email: invite.email,
                role: (0, roles_1.normalizeWorkspaceRole)(invite.role),
    ...
    fastify.delete('/api/invites/:id', {
        ...
        try {
            ...
            if (relatedAuthUser && !relatedAuthUser.email_confirmed_at) {
                try {
                    await supabase_1.supabaseAdmin.auth.admin.deleteUser(relatedAuthUser.id);
                }
                catch (deleteError) {
    ...
    fastify.post('/api/invites/:id/resend', {
        preHandler: [rateLimit_1.rateLimitMiddleware, tenantContext_1.tenantContextMiddleware, auth_1.authMiddleware, membershipGuard_1.membershipGuard],
    }, async (request, reply) => {
        try {
            const tenant = request.tenant;
            ...
            if (existingAuthUser && !existingAuthUser.email_confirmed_at) {
                const { error: deleteAuthUserError } = await supabase_1.supabaseAdmin.auth.admin.deleteUser(existingAuthUser.id);
                if (deleteAuthUserError) {
                    fastify.log.warn({ error: deleteAuthUserError, inviteId: inviteRecord.id, authUserId: existingAuthUser.id }, 'Failed to delete stale unconfirmed auth user before resending invite');
                }
                else {
                    existingAuthUser = null;
                    await supabase_1.supabaseAdmin
    ...
    fastify.post('/api/invites/:id/decline', {
        preHandler: [rateLimit_1.rateLimitMiddleware],
    }, async (request, reply) => {
        try {
            const { id } = request.params;
            ...
            const { error: declineError } = await supabase_1.supabaseAdmin
                .from('workspace_invites')
                .update({
                status: 'declined',
            ...
            fastify.log.error({ error, inviteId: request.params.id }, 'Unexpected error in POST /api/invites/:id/decline');
            if (error instanceof Error) {
                return reply.code(500).send({
                    error: {
                        code: 'INTERNAL_ERROR',
