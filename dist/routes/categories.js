 ...
 */
const deleteCategoryQuerySchema = zod_1.z.object({
    migrate_to_category_id: zod_1.z.string().uuid().nullable().optional(),
});
/**
...
async function categoryRoutes(fastify) {
    ...
    fastify.patch('/api/categories/:id', {
        ...
        try {
            ...
            // Update category
            const { data: category, error: updateError } = await supabase_1.supabaseAdmin
                .from('categories')
                .update(updateData)
                .eq('id', id)
                .eq('tenant_id', tenant.id)
     ...
     */
    fastify.get('/api/categories/:id/documents', {
        preHandler: [rateLimit_1.rateLimitMiddleware, tenantContext_1.tenantContextMiddleware, auth_1.authMiddleware, membershipGuard_1.membershipGuard],
    }, async (request, reply) => {
        try {
            const tenant = request.tenant;
     ...
     */
    fastify.get('/api/categories/uncategorized', {
        preHandler: [rateLimit_1.rateLimitMiddleware, tenantContext_1.tenantContextMiddleware, auth_1.authMiddleware, membershipGuard_1.membershipGuard],
    }, async (request, reply) => {
        try {
            const tenant = request.tenant;
            ...
            const { data: categoryId, error: rpcError } = await supabase_1.supabaseAdmin
    ...
    fastify.delete('/api/categories/:id', {
        ...
        try {
            ...
            if (migrate_to_category_id) {
                const { data: targetCategory, error: targetError } = await supabase_1.supabaseAdmin
                    .from('categories')
                    .select('id, tenant_id')
            ...
            // Count subcategories
            const { count: subcategoryCount } = await supabase_1.supabaseAdmin
                .from('categories')
                .select('id', { count: 'exact', head: true })
                .eq('parent_id', id)
                .eq('tenant_id', tenant.id);
            // Use the RPC function to safely delete with migration
            const { data: result, error: deleteError } = await supabase_1.supabaseAdmin
            ...
            if (migratedToId) {
                const { data: targetCat } = await supabase_1.supabaseAdmin
                    .from('categories')
                    .select('id, name')
                    .eq('id', migratedToId)
                    .single();
