...
async function gdprRoutes(fastify) {
     ...
     */
    fastify.get('/api/gdpr/export', { preHandler: auth_1.authMiddleware }, async (request, reply) => {
        const user = request.user;
        const userId = user.id;
        const tenantId = user.tenant_id;
        try {
            fastify.log.info({ userId, tenantId }, 'Starting GDPR data export');
            // 1. Fetch user profile data
            const { data: userProfile, error: userError } = await supabase_1.supabaseAdmin
                .from('users')
            ...
            const { data: tenantData, error: tenantError } = await supabase_1.supabaseAdmin
                .from('tenants')
            ...
            const { data: documents, error: documentsError } = await supabase_1.supabaseAdmin
                .from('documents')
            ...
            const { data: usageHistory, error: usageError } = await supabase_1.supabaseAdmin
                .from('query_usage')
            ...
            const { data: templates, error: templatesError } = await supabase_1.supabaseAdmin
                .from('templates')
            ...
            fastify.log.error({ userId, tenantId, error }, 'GDPR data export failed');
     ...
     */
    fastify.delete('/api/gdpr/delete-account', { preHandler: auth_1.authMiddleware }, async (request, reply) => {
        const user = request.user;
        const userId = user.id;
        const tenantId = user.tenant_id;
        const DeleteAccountSchema = zod_1.z.object({
            confirmation_token: zod_1.z.string().min(1, 'Confirmation token is required'),
        });
        try {
            const body = DeleteAccountSchema.parse(request.body);
            fastify.log.info({ userId, tenantId }, 'Account deletion request received');
            // Validate confirmation token (must match user ID for security)
            if (body.confirmation_token !== userId) {
                fastify.log.warn({ userId, providedToken: body.confirmation_token }, 'Invalid confirmation token for account deletion');
            ...
            const { data: existingUser, error: userCheckError } = await supabase_1.supabaseAdmin
                .from('users')
                .select('status')
                .eq('id', userId)
                .single();
            ...
            fastify.log.info({ userId, tenantId, jobId: job.id }, 'GDPR deletion job dispatched');
            ...
            fastify.log.error({ userId, tenantId, error }, 'Account deletion failed');
            return reply.status(500).send({
                error: {
     ...
     */
    fastify.patch('/api/user/consents', { preHandler: auth_1.authMiddleware }, async (request, reply) => {
        const user = request.user;
        const userId = user.id;
        const tenantId = user.tenant_id;
        const UpdateConsentsSchema = zod_1.z.object({
            ai_processing: zod_1.z.boolean().optional(),
            analytics_tracking: zod_1.z.boolean().optional(),
            knowledge_indexing: zod_1.z.boolean().optional(),
        }).refine((data) => Object.keys(data).length > 0, { message: 'At least one consent field must be provided' });
        try {
            const body = UpdateConsentsSchema.parse(request.body);
            fastify.log.info({ userId, tenantId, consents: body }, 'Consent update request received');
            // Check if user has existing consent record
            const { data: existingConsent, error: checkError } = await supabase_1.supabaseAdmin
                .from('user_consents')
                .select('*')
                .eq('user_id', userId)
                .single();
            ...
            if (!existingConsent) {
                ...
                const { data: newConsent, error: insertError } = await supabase_1.supabaseAdmin
                    .from('user_consents')
                    .insert({
                    user_id: userId,
                ...
                const { data: updatedConsent, error: updateError } = await supabase_1.supabaseAdmin
                    .from('user_consents')
                    .update(body)
                    .eq('user_id', userId)
                    .select()
            ...
            fastify.log.info({ userId, tenantId }, 'Consent update logged to audit trail');
            ...
            fastify.log.error({ userId, tenantId, error }, 'Consent update failed');
            return reply.status(500).send({
                error: {
     ...
     */
    fastify.get('/api/user/consents', { preHandler: auth_1.authMiddleware }, async (request, reply) => {
        const user = request.user;
        const userId = user.id;
        try {
            const { data: consents, error } = await supabase_1.supabaseAdmin
                .from('user_consents')
