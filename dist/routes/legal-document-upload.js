...
exports.default = legalDocumentUploadRoutes;
...
function resolveMimeType(mimetype, extension) {
    if (mimetype === 'application/octet-stream') {
        return EXTENSION_TO_MIME[extension.toLowerCase()] || mimetype;
    }
    return mimetype;
}
function getAllAllowedMimeTypes() {
    return Object.values(ALLOWED_MIME_TYPES).flat();
}
function getAllAllowedExtensions() {
...
function computeChecksum(buffer, algorithm = 'sha256') {
    return crypto.createHash(algorithm).update(buffer).digest('hex');
}
function extractFileMetadata(filename, mimetype, buffer) {
 ...
 */
async function legalDocumentUploadRoutes(fastify) {
    fastify.post('/api/ai/legal-document/upload', {
        preHandler: [
    ...
    fastify.get('/api/ai/legal-document/supported-types', {
        preHandler: [
            rateLimit_1.rateLimitMiddleware,
            tenantContext_1.tenantContextMiddleware,
            auth_1.authMiddleware,
...
//# sourceMappingURL=legal-document-upload.js.map