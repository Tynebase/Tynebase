...
exports.default = superAdminChangeTierRoutes;
 ...
 */
const changeTierBodySchema = zod_1.z.object({
    tier: zod_1.z.enum(['free', 'base', 'pro', 'enterprise']),
    customCredits: zod_1.z.number().int().positive().optional(),
});
/**
 ...
 */
async function superAdminChangeTierRoutes(fastify) {
    /**
     ...
     */
    fastify.patch('/api/superadmin/tenants/:tenantId/tier', {
        ...
        try {
            ...
            const { data: currentPool, error: poolError } = await supabase_1.supabaseAdmin
                .from('credit_pools')
            ...
            if (currentPool && !poolError) {
                ...
                const { data: pool, error: updatePoolError } = await supabase_1.supabaseAdmin
                    .from('credit_pools')
                ...
                const { data: pool, error: createPoolError } = await supabase_1.supabaseAdmin
                    .from('credit_pools')
                    .insert({
                    tenant_id: tenantId,
...
//# sourceMappingURL=superadmin-change-tier.js.map