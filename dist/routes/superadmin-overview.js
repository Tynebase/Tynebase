...
exports.default = superAdminOverviewRoutes;
 ...
 */
async function superAdminOverviewRoutes(fastify) {
    /**
     ...
     */
    fastify.get('/api/superadmin/overview', {
        ...
        try {
            ...
            const { count: totalTenants, error: tenantsError } = await supabase_1.supabaseAdmin
                .from('tenants')
            ...
            // Query 2: Total users
            const { count: totalUsers, error: usersError } = await supabase_1.supabaseAdmin
                .from('users')
                .select('*', { count: 'exact', head: true })
                .neq('status', 'deleted');
            if (usersError) {
            ...
            const { count: totalDocuments, error: documentsError } = await supabase_1.supabaseAdmin
                .from('documents')
                .select('*', { count: 'exact', head: true });
            if (documentsError) {
            ...
            const { count: activeUsers, error: activeUsersError } = await supabase_1.supabaseAdmin
                .from('users')
                .select('*', { count: 'exact', head: true })
            ...
            const { data: queryData, error: queryError } = await supabase_1.supabaseAdmin
                .from('query_usage')
...
//# sourceMappingURL=superadmin-overview.js.map