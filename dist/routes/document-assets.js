...
exports.default = documentAssetRoutes;
 ...
 */
const listAssetsParamsSchema = zod_1.z.object({
    id: zod_1.z.string().uuid(),
});
/**
 * Zod schema for DELETE /api/documents/:id/assets/:assetId path parameters
 */
const deleteAssetParamsSchema = zod_1.z.object({
    id: zod_1.z.string().uuid(),
 ...
 */
async function documentAssetRoutes(fastify) {
    /**
     ...
     */
    fastify.post('/api/documents/:id/upload', {
        ...
        try {
            ...
            const { error: docError } = await supabase_1.supabaseAdmin
                .from('documents')
                .select('id')
                .eq('id', documentId)
            ...
            if (docError) {
                if (docError.code === 'PGRST116') {
                    fastify.log.warn({ documentId, tenantId: tenant.id, userId: user.id }, 'Document not found or access denied');
                ...
                fastify.log.error({ error: docError, documentId, tenantId: tenant.id, userId: user.id }, 'Failed to fetch document for asset upload');
     ...
     */
    fastify.get('/api/documents/:id/assets', {
        preHandler: [rateLimit_1.rateLimitMiddleware, tenantContext_1.tenantContextMiddleware, auth_1.authMiddleware, membershipGuard_1.membershipGuard],
    }, async (request, reply) => {
        try {
            const tenant = request.tenant;
            ...
            if (docError) {
                ...
                fastify.log.error({ error: docError, documentId, tenantId: tenant.id, userId: user.id }, 'Failed to fetch document for asset listing');
                return reply.code(500).send({
                    error: {
                        code: 'FETCH_FAILED',
                        message: 'Failed to fetch document',
            ...
            const { data: files, error: listError } = await supabase_1.supabaseAdmin
                .storage
                .from('tenant-documents')
                .list(storagePath, {
                limit: 1000,
            ...
            const assets = await Promise.all((files || []).map(async (file) => {
                ...
                const { data: signedUrlData } = await supabase_1.supabaseAdmin
                    .storage
                    .from('tenant-documents')
     ...
     */
    fastify.delete('/api/documents/:id/assets/:assetId', {
        preHandler: [rateLimit_1.rateLimitMiddleware, tenantContext_1.tenantContextMiddleware, auth_1.authMiddleware, membershipGuard_1.membershipGuard],
    }, async (request, reply) => {
        try {
            const tenant = request.tenant;
            ...
            if (docError) {
                ...
                fastify.log.error({ error: docError, documentId, tenantId: tenant.id, userId: user.id }, 'Failed to fetch document for asset deletion');
                return reply.code(500).send({
                    error: {
                        code: 'FETCH_FAILED',
                        message: 'Failed to fetch document',
     ...
     */
    fastify.get('/api/documents/:id/assets/serve/*', {
        ...
        try {
            ...
            let { data: fileData, error: downloadError } = await supabase_1.supabaseAdmin
                .storage
                .from('tenant-documents')
                .download(storagePath);
            // If exact match fails, search for file with timestamp prefix (handles old URLs)
            if (downloadError || !fileData) {
                const { data: files } = await supabase_1.supabaseAdmin
                    .storage
                    .from('tenant-documents')
                    .list(basePath);
                if (files && files.length > 0) {
                    ...
                    const matchingFile = files.find(f => {
                        ...
                        if (match && match[1] === assetFilename)
                            return true;
                        return f.name === assetFilename;
                    });
                    if (matchingFile) {
                        storagePath = `${basePath}/${matchingFile.name}`;
                        const retryResult = await supabase_1.supabaseAdmin
                            .storage
                            .from('tenant-documents')
                            .download(storagePath);
     ...
     */
    fastify.get('/api/documents/:id/assets/public/*', {
        preHandler: [rateLimit_1.rateLimitMiddleware],
    }, async (request, reply) => {
        try {
            const params = request.params;
...
//# sourceMappingURL=document-assets.js.map