...
const ScrapeRequestSchema = zod_1.z.object({
    ...
    output_types: zod_1.z.array(zod_1.z.enum(['full_article', 'summary', 'outline', 'raw'])).min(1).max(4).optional().default(['full_article']),
    ai_model: zod_1.z.enum(['deepseek', 'claude', 'gemini'])
        .optional()
        .default('deepseek'),
    timeout: zod_1.z.number().int().min(1000).max(30000).optional().default(15000),
});
const SCRAPE_BASE_CREDITS = 2;
function getPromptForOutputType(outputType, content, title) {
    switch (outputType) {
        case 'summary':
 ...
 * POST /api/ai/scrape
 */
async function aiScrapeRoutes(fastify) {
    fastify.post('/api/ai/scrape', {
        preHandler: [
        ...
        try {
            ...
            const aiModelCost = (0, creditCalculator_1.getModelCreditCost)(validated.ai_model);
            ...
            if (deductError) {
                request.log.error({ tenantId: tenant.id, error: deductError.message }, 'Failed to deduct credits');
                return reply.status(500).send({
                    error: {
            ...
            const scrapeResult = await (0, tavily_1.scrapeUrlToMarkdown)(validated.url, validated.timeout);
            request.log.info({
                tenantId: tenant.id,
                userId: user.id,
            ...
            for (const outputType of validated.output_types) {
                ...
                if (outputType === 'raw') {
                    ...
                    const prompt = getPromptForOutputType(outputType, scrapeResult.markdown, scrapeResult.title);
                    request.log.info({ tenantId: tenant.id, aiModel: validated.ai_model, outputType }, 'Processing scraped content with AI');
                    const aiResponse = await (0, generation_1.generateText)({
                        prompt,
                ...
                if (!isOnlyRaw) {
                    ...
                    if (!docError && doc) {
                        ...
                        request.log.info({ documentId: doc.id, outputType }, 'Document created from scraped content');
                    }
                }
            }
            request.log.info({ tenantId: tenant.id, totalTokensUsed, documentsCreated: documentIds.length }, 'Scrape processing completed');
            // Step 4: Log usage
...
//# sourceMappingURL=ai-scrape.js.map