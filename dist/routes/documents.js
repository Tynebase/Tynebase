 ...
 */
function rewriteAssetUrlsForServe(content, documentId, apiBaseUrl) {
    if (!content)
        return content;
    const supabaseUrlPattern = /https?:\/\/[^"'\s]+\.supabase\.co\/storage\/v1\/object\/sign\/tenant-documents\/tenant-[^/]+\/documents\/([^/]+)\/([^"'\s?]+)[^"'\s]*/g;
    return content.replace(supabaseUrlPattern, (match, docIdFromUrl, filename) => {
 ...
 */
const getDocumentParamsSchema = zod_1.z.object({
    id: zod_1.z.string().uuid(),
});
/**
 * Zod schema for POST /api/documents request body
 */
const createDocumentBodySchema = zod_1.z.object({
    title: zod_1.z.string().min(1).max(500),
    content: zod_1.z.string().optional(),
    category_id: zod_1.z.string().uuid().nullable().optional(),
    is_public: zod_1.z.boolean().default(false), // Deprecated, use visibility
    visibility: zod_1.z.enum(['private', 'team', 'public', 'community']).default('team'),
});
/**
 * Zod schema for PATCH /api/documents/:id request body
 */
const updateDocumentBodySchema = zod_1.z.object({
    title: zod_1.z.string().min(1).max(500).optional(),
    content: zod_1.z.string().max(10485760).optional(), // Max 10MB content
    yjs_state: zod_1.z.string().optional(), // Base64 encoded binary state
    is_public: zod_1.z.boolean().optional(), // Deprecated, use visibility
    visibility: zod_1.z.enum(['private', 'team', 'public', 'community']).optional(),
    status: zod_1.z.enum(['draft', 'published']).optional(),
    category_id: zod_1.z.string().uuid().nullable().optional(), // Category/folder ID
    draft_content: zod_1.z.string().max(10485760).optional(), // Draft content for published docs
    draft_title: zod_1.z.string().min(1).max(500).optional(), // Draft title
    save_as_draft: zod_1.z.boolean().optional(), // Flag to indicate saving to draft vs publishing
 ...
 */
const updateDocumentParamsSchema = zod_1.z.object({
    id: zod_1.z.string().uuid(),
});
/**
 * Zod schema for DELETE /api/documents/:id path parameters
 */
const deleteDocumentParamsSchema = zod_1.z.object({
    id: zod_1.z.string().uuid(),
});
/**
 * Zod schema for POST /api/documents/:id/publish path parameters
 */
const publishDocumentParamsSchema = zod_1.z.object({
    id: zod_1.z.string().uuid(),
});
/**
 * Zod schema for POST /api/documents/:id/publish request body
 */
const publishDocumentBodySchema = zod_1.z.object({
    visibility: zod_1.z.enum(['private', 'team', 'public', 'community']).optional(),
}).optional();
/**
 * Zod schema for GET /api/documents/:id/normalized path parameters
 */
const getNormalizedParamsSchema = zod_1.z.object({
    id: zod_1.z.string().uuid(),
});
/**
...
async function documentRoutes(fastify) {
    ...
    fastify.get('/api/documents', {
        ...
        try {
          ...
          `, { count: 'exact' })
            ...
            if (error) {
                fastify.log.error({ error, tenantId: tenant.id, userId: user.id }, 'Failed to fetch documents');
            ...
            if (documentIds.length > 0) {
                const { data: collectionDocs } = await supabase_1.supabaseAdmin
            ...
            `)
                ...
                const { data: tagDocs } = await supabase_1.supabaseAdmin
                    .from('document_tags')
                    .select(`
              document_id,
              tags:tag_id (
    ...
    fastify.get('/api/documents/:id', {
        ...
        try {
            ...
            let { data: document, error } = await supabase_1.supabaseAdmin
                .from('documents')
                .select(`
            id,
            title,
          ...
          `)
            ...
            if (error) {
                ...
                fastify.log.error({ error, documentId: id, tenantId: tenant.id, userId: user.id }, 'Failed to fetch document');
                return reply.code(500).send({
                    error: {
                        code: 'FETCH_FAILED',
            ...
            fastify.log.info({ documentId: id, tenantId: tenant.id, userId: user.id, isCrossTenant }, 'Document fetched successfully');
            // Increment view count unless skipped (fire and forget - don't block response)
            const skipViewIncrement = request.query?.skip_view_increment === 'true';
    ...
    fastify.post('/api/documents', {
        ...
        try {
            ...
            if (category_id) {
                const { data: category, error: categoryError } = await supabase_1.supabaseAdmin
                    .from('categories')
                    .select('id')
                    .eq('id', category_id)
                    .eq('tenant_id', tenant.id)
                ...
                if (categoryError || !category) {
                    fastify.log.warn({ categoryId: category_id, tenantId: tenant.id, userId: user.id }, 'Category not found or belongs to different tenant');
                    return reply.code(400).send({
                        error: {
          ...
          `)
                .single();
            if (createError) {
                fastify.log.error({ error: createError, tenantId: tenant.id, userId: user.id }, 'Failed to create document');
                return reply.code(500).send({
                    error: {
                        code: 'CREATE_FAILED',
            ...
            if (lineageError) {
                fastify.log.error({ error: lineageError, documentId: document.id, userId: user.id }, 'Failed to create lineage event');
            }
            fastify.log.info({ documentId: document.id, tenantId: tenant.id, userId: user.id, title }, 'Document created successfully');
    ...
    fastify.patch('/api/documents/:id', {
        ...
        try {
            ...
            const { data: existingDoc, error: fetchError } = await supabase_1.supabaseAdmin
                .from('documents')
            ...
            if (fetchError) {
                ...
                fastify.log.error({ error: fetchError, documentId: id, tenantId: tenant.id, userId: user.id }, 'Failed to fetch document for update');
                return reply.code(500).send({
                    error: {
                        code: 'FETCH_FAILED',
                        message: 'Failed to fetch document',
            ...
            if (isPublished && isDraftSave) {
                ...
                if (yjs_state !== undefined) {
                    try {
                        ...
                        fastify.log.warn({ documentId: id, userId: user.id }, 'Invalid base64 encoding for yjs_state');
                        return reply.code(400).send({
                            error: {
            ...
            // Update document
            const { data: updatedDoc, error: updateError } = await supabase_1.supabaseAdmin
                .from('documents')
                .update(updateData)
                .eq('id', id)
                .eq('tenant_id', tenant.id)
          ...
          `)
                .single();
            if (updateError) {
                fastify.log.error({ error: updateError, documentId: id, tenantId: tenant.id, userId: user.id }, 'Failed to update document');
                return reply.code(500).send({
                    error: {
                        code: 'UPDATE_FAILED',
            ...
            if (lineageError) {
                fastify.log.error({ error: lineageError, documentId: updatedDoc.id, userId: user.id }, 'Failed to create lineage event for document update');
            }
            ...
            if (statusChangedToPublished) {
                try {
                    ...
                    const { data: existingJobs, error: jobCheckError } = await supabase_1.supabaseAdmin
                        .from('job_queue')
                        .select('id, status')
                        .eq('tenant_id', tenant.id)
                        .eq('type', 'rag_index')
                    ...
                    if (jobCheckError) {
                        fastify.log.error({ error: jobCheckError, documentId: updatedDoc.id }, 'Failed to check for existing rag_index jobs');
                    }
                    else if (existingJobs && existingJobs.length > 0) {
                        fastify.log.info({ documentId: updatedDoc.id, existingJobId: existingJobs[0].id }, 'Skipping rag_index job dispatch - job already pending/processing');
                    }
                    else {
                    ...
                    fastify.log.error({ error: dispatchError, documentId: updatedDoc.id }, 'Failed to dispatch rag_index job - document saved but indexing may be delayed');
                }
            }
            fastify.log.info({
    ...
    fastify.delete('/api/documents/:id', {
        ...
        try {
            ...
            if (fetchError) {
                ...
                fastify.log.error({ error: fetchError, documentId: id, tenantId: tenant.id, userId: user.id }, 'Failed to fetch document for deletion');
                return reply.code(500).send({
                    error: {
                        code: 'FETCH_FAILED',
                        message: 'Failed to fetch document',
            ...
            const { error: deleteError } = await supabase_1.supabaseAdmin
                ...
                .eq('tenant_id', tenant.id);
            if (deleteError) {
                fastify.log.error({ error: deleteError, documentId: id, tenantId: tenant.id, userId: user.id }, 'Failed to delete document');
                return reply.code(500).send({
                    error: {
                        code: 'DELETE_FAILED',
     ...
     */
    fastify.post('/api/documents/:id/publish', {
        preHandler: [rateLimit_1.rateLimitMiddleware, tenantContext_1.tenantContextMiddleware, auth_1.authMiddleware, membershipGuard_1.membershipGuard],
    }, async (request, reply) => {
        try {
            const tenant = request.tenant;
            ...
            if (!user.is_super_admin && user.role !== 'admin' && user.role !== 'editor') {
                fastify.log.warn({ documentId: id, userId: user.id, userRole: user.role, tenantId: tenant.id }, 'User attempted to publish document without permission');
                return reply.code(403).send({
                    error: {
                        code: 'FORBIDDEN',
            ...
            if (fetchError) {
                ...
                fastify.log.error({ error: fetchError, documentId: id, tenantId: tenant.id, userId: user.id }, 'Failed to fetch document for publishing');
                return reply.code(500).send({
                    error: {
                        code: 'FETCH_FAILED',
                        message: 'Failed to fetch document',
            ...
            if (!categoryId) {
                ...
                const { data: defaultCategory } = await supabase_1.supabaseAdmin
                    .from('categories')
                    .select('id')
                    .eq('tenant_id', tenant.id)
                    .eq('name', 'Default')
                ...
                if (defaultCategory) {
                    ...
                    const { data: newCategory, error: createCategoryError } = await supabase_1.supabaseAdmin
                        .from('categories')
                        .insert({
                        name: 'Default',
                    ...
                    if (createCategoryError) {
                        fastify.log.error({ error: createCategoryError, tenantId: tenant.id }, 'Failed to create default category');
                    }
                    else {
                        categoryId = newCategory.id;
                        fastify.log.info({ categoryId: newCategory.id, tenantId: tenant.id }, 'Created default category for published document');
                    }
                }
            }
            ...
            const { data: publishedDoc, error: updateError } = await supabase_1.supabaseAdmin
                .from('documents')
                .update(updateData)
                .eq('id', id)
                .eq('tenant_id', tenant.id)
          ...
          `)
                .single();
            if (updateError) {
                fastify.log.error({ error: updateError, documentId: id, tenantId: tenant.id, userId: user.id }, 'Failed to publish document');
                return reply.code(500).send({
                    error: {
                        code: 'PUBLISH_FAILED',
            ...
            if (lineageError) {
                fastify.log.error({ error: lineageError, documentId: publishedDoc.id, userId: user.id }, 'Failed to create lineage event for document publication');
            }
            ...
            try {
                ...
                if (jobCheckError) {
                    fastify.log.error({ error: jobCheckError, documentId: publishedDoc.id }, 'Failed to check for existing rag_index jobs');
                }
                else if (existingJobs && existingJobs.length > 0) {
                    fastify.log.info({ documentId: publishedDoc.id, existingJobId: existingJobs[0].id }, 'Skipping rag_index job dispatch - job already pending/processing');
                }
                else {
                    // No duplicate job found, dispatch new rag_index job
                    const job = await (0, dispatchJob_1.dispatchJob)({
                ...
                fastify.log.error({ error: dispatchError, documentId: publishedDoc.id }, 'Failed to dispatch rag_index job - document published but indexing may be delayed');
            }
            fastify.log.info({
     ...
     */
    fastify.post('/api/documents/:id/discard-draft', {
        preHandler: [rateLimit_1.rateLimitMiddleware, tenantContext_1.tenantContextMiddleware, auth_1.authMiddleware, membershipGuard_1.membershipGuard],
    }, async (request, reply) => {
        try {
            const tenant = request.tenant;
          ...
          `)
                .single();
            if (updateError) {
                fastify.log.error({ error: updateError, documentId: id, tenantId: tenant.id, userId: user.id }, 'Failed to discard draft');
                return reply.code(500).send({
                    error: {
                        code: 'DISCARD_FAILED',
            ...
            if (lineageError) {
                fastify.log.error({ error: lineageError, documentId: id, userId: user.id }, 'Failed to create lineage event for draft discard');
            }
            fastify.log.info({ documentId: id, tenantId: tenant.id, userId: user.id }, 'Draft discarded successfully');
            return reply.code(200).send({
                success: true,
                data: {
                    document: updatedDoc,
     ...
     */
    fastify.get('/api/documents/:id/normalized', {
        preHandler: [rateLimit_1.rateLimitMiddleware, tenantContext_1.tenantContextMiddleware, auth_1.authMiddleware, membershipGuard_1.membershipGuard],
    }, async (request, reply) => {
        try {
            const tenant = request.tenant;
            ...
            if (error) {
                ...
                fastify.log.error({ error, documentId: id, tenantId: tenant.id, userId: user.id }, 'Failed to fetch normalized content');
                return reply.code(500).send({
                    error: {
                        code: 'FETCH_FAILED',
            ...
            fastify.log.info({ documentId: id, tenantId: tenant.id, userId: user.id }, 'Normalized content fetched successfully');
            return reply.code(200).send({
                success: true,
                data: {
                    id: document.id,
     ...
     */
    fastify.get('/api/documents/:id/versions', {
        preHandler: [rateLimit_1.rateLimitMiddleware, tenantContext_1.tenantContextMiddleware, auth_1.authMiddleware, membershipGuard_1.membershipGuard],
    }, async (request, reply) => {
        try {
            const tenant = request.tenant;
            ...
            const { data: versions, error: versionsError } = await supabase_1.supabaseAdmin
            ...
            fastify.log.info({ documentId: id, versionCount: mappedVersions.length }, 'Document versions fetched');
            return reply.code(200).send({
                success: true,
     ...
     */
    fastify.post('/api/documents/:id/versions/:versionId/restore', {
        preHandler: [rateLimit_1.rateLimitMiddleware, tenantContext_1.tenantContextMiddleware, auth_1.authMiddleware, membershipGuard_1.membershipGuard],
    }, async (request, reply) => {
        try {
            const tenant = request.tenant;
            ...
            const { data: version, error: versionError } = await supabase_1.supabaseAdmin
                .from('document_versions')
            ...
            fastify.log.info({ documentId: id, versionId, userId: user.id }, 'Document version restored');
            return reply.code(200).send({
                success: true,
    ...
    // ============================================================================
    const videoIngestionBodySchema = zod_1.z.object({
     ...
     */
    fastify.post('/api/documents/:id/videos/ingest', {
        preHandler: [
            rateLimit_1.rateLimitMiddleware,
            tenantContext_1.tenantContextMiddleware,
            auth_1.authMiddleware,
        ...
        try {
            ...
            if (options.generate_summary)
                creditsPerVideo += modelCost;
            if (options.generate_article)
                creditsPerVideo += modelCost;
            ...
            fastify.log.info({ documentId: id, videoCount: videos.length, totalCredits, userId: user.id }, 'Document video ingestion started');
            return reply.code(200).send({
                success: true,
                data: {
                    documentId: id,
     ...
     */
    fastify.get('/api/public/documents', { preHandler: [rateLimit_1.rateLimitMiddleware] }, async (request, reply) => {
        try {
          ...
          `, { count: 'exact' })
            ...
            const finalDocuments = documentsWithRewrittenUrls.map((doc) => ({
                ...doc,
                tags: documentTags[doc.id] || [],
            }));
            // Fetch available filter options (tenants, categories, tags that have public docs)
            // Tenants that have public documents
            const { data: tenantOptions } = await supabase_1.supabaseAdmin
                .from('documents')
            ...
            const { data: categoryOptions } = await supabase_1.supabaseAdmin
                .from('documents')
            ...
            if (categoryOptions) {
                for (const d of categoryOptions) {
            ...
            const { data: publicDocIds } = await supabase_1.supabaseAdmin
                .from('documents')
                .select('id')
                .in('visibility', ['public', 'community'])
                .eq('status', 'published')
            ...
            if (publicDocIds && publicDocIds.length > 0) {
                const { data: tagOptions } = await supabase_1.supabaseAdmin
                    .from('document_tags')
                    .select('tag_id, tags:tag_id (id, name, description)')
                    .in('document_id', publicDocIds.map((d) => d.id));
                if (tagOptions) {
     ...
     */
    fastify.get('/api/public/documents/:id', { preHandler: [rateLimit_1.rateLimitMiddleware] }, async (request, reply) => {
        try {
