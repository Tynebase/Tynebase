...
exports.default = superAdminTenantsRoutes;
...
const querySchema = zod_1.z.object({
    ...
    limit: zod_1.z.coerce.number().int().positive().max(100).default(100),
});
/**
 ...
 */
async function superAdminTenantsRoutes(fastify) {
    /**
    ...
    fastify.get('/api/superadmin/tenants', {
        ...
        try {
            ...
            const { data: tenants, error: tenantsError, count: totalCount } = await supabase_1.supabaseAdmin
                .from('tenants')
            ...
            if (tenantsError) {
                ...
                const is416Error = errorCode === 'PGRST103' ||
                    errorCode === '416' ||
            ...
            const { data: userCounts, error: userCountsError } = await supabase_1.supabaseAdmin
                .from('users')
                .select('tenant_id')
            ...
            const { data: documentCounts, error: documentCountsError } = await supabase_1.supabaseAdmin
                .from('documents')
                .select('tenant_id')
                .in('tenant_id', tenantIds);
            ...
            const { data: creditPools, error: creditPoolsError } = await supabase_1.supabaseAdmin
                .from('credit_pools')
            ...
            const { data: lastActiveData, error: lastActiveError } = await supabase_1.supabaseAdmin
                .from('users')
...
//# sourceMappingURL=superadmin-tenants.js.map