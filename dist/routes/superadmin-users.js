...
exports.default = superAdminUsersRoutes;
...
const listUsersQuerySchema = zod_1.z.object({
    ...
    filter: zod_1.z.enum(['new30d', 'active7d']).optional(),
});
/**
 ...
 */
const assignCreditsBodySchema = zod_1.z.object({
    credits: zod_1.z.number().int().min(1).max(100000),
});
/**
 ...
 */
async function superAdminUsersRoutes(fastify) {
    /**
     * GET /api/superadmin/users
     *
     * Returns paginated list of all users across all tenants
     */
    fastify.get('/api/superadmin/users', {
        ...
        try {
          ...
          `, { count: 'exact' })
            ...
            if (tenantIds.length > 0) {
                const { data: tenants } = await supabase_1.supabaseAdmin
                    .from('tenants')
     ...
     */
    fastify.delete('/api/superadmin/users/:userId', {
        preHandler: [auth_1.authMiddleware, superAdminGuard_1.superAdminGuard],
    }, async (request, reply) => {
        try {
            ...
            request.log.info({ superAdminId: request.user?.id, archivedUserId: userId, archivedEmail: targetUser.email }, 'Super admin archived user');
            return {
                success: true,
     ...
     */
    fastify.post('/api/superadmin/users/:userId/restore', {
        preHandler: [auth_1.authMiddleware, superAdminGuard_1.superAdminGuard],
    }, async (request, reply) => {
        try {
            const { userId } = userIdParamsSchema.parse(request.params);
            ...
            request.log.info({ superAdminId: request.user?.id, restoredUserId: userId, restoredEmail: targetUser.email }, 'Super admin re-instated user');
            return {
                success: true,
     ...
     */
    fastify.post('/api/superadmin/users/:userId/recovery', {
        preHandler: [auth_1.authMiddleware, superAdminGuard_1.superAdminGuard],
    }, async (request, reply) => {
        try {
            const { userId } = userIdParamsSchema.parse(request.params);
            ...
            const { error: resetError } = await supabase_1.supabaseAdmin.auth.resetPasswordForEmail(targetUser.email, {
                redirectTo: `${process.env.FRONTEND_URL || 'https://www.tynebase.com'}/auth/update-password`,
            });
            if (resetError) {
                request.log.error({ error: resetError, email: targetUser.email }, 'Failed to send password recovery email');
                return reply.status(500).send({
                    error: { code: 'EMAIL_SEND_FAILED', message: `Failed to send recovery email: ${resetError.message}` },
                });
            }
            request.log.info({ superAdminId: request.user?.id, targetUserId: userId, targetEmail: targetUser.email }, 'Super admin sent password recovery email');
            return {
                success: true,
     ...
     */
    fastify.post('/api/superadmin/users/:userId/credits', {
        preHandler: [auth_1.authMiddleware, superAdminGuard_1.superAdminGuard],
    }, async (request, reply) => {
        try {
            const { userId } = userIdParamsSchema.parse(request.params);
            ...
            const { data: pool, error: poolError } = await supabase_1.supabaseAdmin
                .from('credit_pools')
     ...
     */
    fastify.get('/api/superadmin/kpis', {
        preHandler: [auth_1.authMiddleware, superAdminGuard_1.superAdminGuard],
    }, async (request, reply) => {
        try {
            const now = new Date();
            ...
            const [tenantsResult, usersResult, activeUsersResult, documentsResult, newUsersResult, newDocsResult, aiQueriesResult, creditPoolsResult,] = await Promise.all([
                supabase_1.supabaseAdmin.from('tenants').select('*', { count: 'exact', head: true }),
                supabase_1.supabaseAdmin.from('users').select('*', { count: 'exact', head: true }).neq('status', 'suspended'),
                supabase_1.supabaseAdmin.from('users').select('*', { count: 'exact', head: true }).gte('last_active_at', sevenDaysAgo.toISOString()).neq('status', 'suspended'),
                supabase_1.supabaseAdmin.from('documents').select('*', { count: 'exact', head: true }),
                supabase_1.supabaseAdmin.from('users').select('*', { count: 'exact', head: true }).gte('created_at', thirtyDaysAgo.toISOString()).neq('status', 'suspended'),
                supabase_1.supabaseAdmin.from('documents').select('*', { count: 'exact', head: true }).gte('created_at', thirtyDaysAgo.toISOString()),
                supabase_1.supabaseAdmin.from('query_usage').select('credits_charged').gte('created_at', thirtyDaysAgo.toISOString()),
                supabase_1.supabaseAdmin.from('credit_pools').select('total_credits, used_credits'),
            ]);
...
//# sourceMappingURL=superadmin-users.js.map