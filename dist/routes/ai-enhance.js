...
exports.default = aiEnhanceRoutes;
...
const EnhanceRequestSchema = zod_1.z.object({
    document_id: zod_1.z.string().uuid('Invalid document ID format'),
    custom_prompt: zod_1.z.string().optional(),
    editor_content: zod_1.z.string().optional(),
});
/**
 ...
 */
async function aiEnhanceRoutes(fastify) {
    fastify.post('/api/ai/enhance', {
        preHandler: [
        ...
        try {
            ...
            const creditsToDeduct = (0, creditCalculator_1.getModelCreditCost)('claude');
...
- Do NOT include fields that aren't needed for the action type`;
            ...
            let aiResponse;
            try {
                aiResponse = await (0, anthropic_1.generateText)({
                    prompt,
            ...
            try {
                ...
                const normalizeEnhanceResult = (parsed) => {
                    ...
                    const suggestions = suggestionsRaw
                        ...
                        .filter((s) => s && ['add', 'replace', 'delete'].includes(s.action));
                ...
                const validateSuggestionsAgainstContent = (result, content) => {
                    const validated = result.suggestions.filter((s) => {
                        if ((s.action === 'replace' || s.action === 'delete') && s.find) {
                            if (!content.includes(s.find)) {
                                request.log.warn({ title: s.title, findLength: s.find.length }, 'Dropping suggestion: find text not found in document');
                                return false;
                            }
                        }
                        return true;
    ...
    const ApplySuggestionSchema = zod_1.z.object({
        document_id: zod_1.z.string().uuid('Invalid document ID format'),
        suggestion_title: zod_1.z.string(),
        suggestion_action: zod_1.z.enum(['add', 'replace', 'delete']),
        suggestion_type: zod_1.z.enum(['grammar', 'clarity', 'structure', 'completeness', 'style']),
    });
    fastify.post('/api/ai/enhance/lineage', {
        preHandler: [tenantContext_1.tenantContextMiddleware, auth_1.authMiddleware],
        schema: {
            body: {
                type: 'object',
        ...
        try {
            ...
            if (lineageError) {
                request.log.error({ error: lineageError, documentId: validated.document_id, userId: user.id }, 'Failed to create lineage event for AI enhancement');
                return reply.status(500).send({
                    error: {
            ...
            request.log.error({ error: error instanceof Error ? error.message : 'Unknown error' }, 'Error in AI enhance apply endpoint');
            return reply.status(500).send({
                error: {
                    code: 'INTERNAL_ERROR',
                    message: 'An error occurred while processing your request',
...
//# sourceMappingURL=ai-enhance.js.map