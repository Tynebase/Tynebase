...
exports.sendRoleChangeEmail = sendRoleChangeEmail;
exports.sendUserRemovedEmail = sendUserRemovedEmail;
 ...
 */
async function sendUserRemovedEmail(params) {
...
async function sendWorkspaceInviteEmail(params) {
    const { to, tenantName, role, invitedBy, acceptUrl, declineUrl } = params;
    const content = `
    <h2 style="margin: 0 0 16px; color: #1e293b; font-size: 22px; font-weight: 600;">
    ...
    <div style="text-align: center; margin: 32px 0;">
      <a href="${acceptUrl}" style="display: inline-block; padding: 14px 32px; background-color: #E85002; color: #ffffff; text-decoration: none; font-size: 15px; font-weight: 600; border-radius: 10px; margin-bottom: 12px;">
        Accept Invitation
      </a>
      ${declineUrl ? `
        <br>
        <a href="${declineUrl}" style="display: inline-block; padding: 10px 24px; background-color: transparent; color: #64748b; text-decoration: none; font-size: 13px; font-weight: 500; border: 1px solid #e2e8f0; border-radius: 8px;">
          Decline Invitation
        </a>
      ` : ''}
    </div>
 ...
 */
async function sendUserLeftEmail(params) {
 ...
 */
async function sendUserLeftAdminNotification(params) {
