...
function getVertexClient() {
    if (!vertexClient) {
        ...
        if (credentials) {
            ...
            vertexClient = new vertexai_1.VertexAI({
                project: projectId,
 ...
 */
async function transcribeAudio(audioGcsUri, prompt = 'Transcribe this audio content with timestamps. Include all spoken words and important context.') {
    ...
    try {
        ...
        const inputTokens = (0, tokenCounter_1.countTokens)(prompt, 'gpt-4');
        ...
        const outputTokens = usageMetadata?.candidatesTokenCount || (0, tokenCounter_1.countTokens)(content, 'gpt-4');
        return {
            content,
 ...
 * @throws Error on API failures or timeout
 */
async function transcribeVideo(videoUrl, prompt = 'Transcribe this video content with timestamps. Include all spoken words and important visual context.') {
    ...
    try {
        ...
        // Generic error
        throw new Error(`Vertex AI transcription error: ${error?.message || 'Unknown error'}`);
    }
}
/**
...
async function generateText(request) {
    ...
    try {
        ...
        // Generic error
        throw new Error(`Vertex AI API error: ${error?.message || 'Unknown error'}`);
    }
}
/**
