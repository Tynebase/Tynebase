...
const getModelArn = (modelId, region) => `arn:aws:bedrock:${region}::foundation-model/${modelId}`;
/**
...
function getBedrockClient(region) {
    if (!clients.has(region)) {
        ...
        clients.set(region, new client_bedrock_runtime_1.BedrockRuntimeClient({
            region,
 ...
 */
function getAgentRuntimeClient(region) {
    if (!agentClients.has(region)) {
        ...
        agentClients.set(region, new client_bedrock_agent_runtime_1.BedrockAgentRuntimeClient({
            region,
            credentials: { accessKeyId, secretAccessKey },
            maxAttempts: 3,
        }));
 ...
 */
async function generateEmbedding(text, inputType = 'search_query') {
    ...
    try {
        const payload = {
            ...
            output_dimension: exports.EMBEDDING_DIMENSIONS,
            truncate: 'RIGHT',
        };
 ...
 */
function handleBedrockError(error, operation) {
 ...
 */
async function rerankDocuments(query, documents, topN) {
    ...
    try {
        ...
        const command = new client_bedrock_agent_runtime_1.RerankCommand({
            queries: [
                {
                    type: 'TEXT',
