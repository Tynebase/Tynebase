...
export async function creditGuardMiddleware(
  ...
  try {
    ...
    if (!data || data.length === 0) {
      ...
      request.log.info(
        { tenantId, month: currentMonth, tier: tenant.tier, credits: tierCredits },
        'Auto-initialising monthly credit pool'
      );
      ...
      if (insertError) {
        ...
        if (insertError.code !== '23505') {
          request.log.error(
            { tenantId, error: insertError.message },
            'Failed to auto-initialise credit pool'
          );
          return reply.status(500).send({
            error: {
        ...
        request.log.debug(
          { tenantId },
          'Credit pool already exists (race condition on init) — continuing'
        );
      }
