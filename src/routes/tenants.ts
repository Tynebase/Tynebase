...
const settingsSchema = z.object({
  branding: z.object({
    ...
    kb_eyebrow: z.string().max(100).optional(),
    kb_heading: z.string().max(200).optional(),
    kb_subheading: z.string().max(300).optional(),
  }).optional(),
...
export default async function tenantRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/api/tenants/:id/logo', {
    ...
    try {
      ...
      const { data: publicUrlData } = supabaseAdmin.storage.from('platform-assets').getPublicUrl(storagePath);
      ...
      const merged = {
        ...(existing?.settings ?? {}),
        branding: {
          ...((existing?.settings as any)?.branding ?? {}),
          logo_url: logoUrl,
        },
      };
