 ...
 */
export default async function knowledgeActivityRoutes(fastify: FastifyInstance) {
  /**
    ...
    async (request, reply) => {
      try {
        ...
        if (countResult.error) {
          fastify.log.error(
            { error: countResult.error, tenantId: tenant.id },
        ...
        if (dataResult.error) {
          fastify.log.error(
            { error: dataResult.error, tenantId: tenant.id },
        ...
        const actorIds = [...new Set(rawData.map((item: any) => item.actor_id).filter(Boolean))];
        ...
        const activities: ActivityItem[] = rawData
          .map((item: any) => {
            ...
            return {
              id: item.id,
              type: item.event_type as ActivityType,
              actor: {
                id: item.actor_id,
                name: actorName,
